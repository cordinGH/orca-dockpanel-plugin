...
<img width="313" height="213" alt="PixPin_2026-03-08_20-28-58" src="https://github.com/user-attachments/assets/88cf8f86-1d1e-4385-8621-2196890c31d6" />

- 任意面板左键按钮：切出/隐藏小窗面板
  - 若当前不存在小窗面板，则会新建一个切出来（基于默认块）。
- 右键按钮：将所属面板挂起小窗状态
  - 若当前仅唯一面板，则挂起为小窗后，还会在原地创建一个新面板（基于默认块）。
  - 若当前存在小窗面板，则会与小窗面板交换位置，并继承其隐藏状态。
- 任意面板中键按钮：退出小窗状态（恢复原样）
  - 如果您的鼠标没有中键，则alt + 右键具有相同效果

## 版本主要变更
- ...
- 2.3.0 新增设置选项，自动隐藏停靠面板（脱离焦点的时候）
